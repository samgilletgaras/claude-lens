...
export function parseHash(hash: string): { view: AppView; projectId: string | null; sessionId: string | null } {
  ...
  const projectId = view === 'history' ? (parts[1] ? decodeURIComponent(parts[1]) : null) : null;
  const sessionId = projectId ? (parts[2] ? decodeURIComponent(parts[2]) : null) : null;
...
export function buildHash(view: AppView, projectId: string | null, sessionId: string | null): string {
  ...
  if (!sessionId) return `#/history/${encodeURIComponent(projectId)}`;
  return `#/history/${encodeURIComponent(projectId)}/${encodeURIComponent(sessionId)}`;
}
